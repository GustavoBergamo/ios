...
func coldWarmOrHot(guess: Int) -> String {
    ...
    if guess == randomNumber{
        print("That's insane, Wayne! You've guessed the number exactly and burned yourself! 🔥")
    }else if difference < 11{
        print("Great hypothesis, Papadopoulos! You're hot!!! 🥵")
    }else if difference < 26{
        print("Oh my, Bill Nye! You're warmer! ☺️")
    }else{
        print("No way Jose. You're cold. 🥶")
    }
...
func countToOneHundred(count: Int) -> Int {
    var a = count + 1
    if a < 100{
        return countToOneHundred(count: a)}
    else{
        return a
