...
var ages: [Int] = [1, 2, 3]
//colocando uma condição caso esteja vazio
if let ages2 = ages.last {
    print("O último número é: \(ages2)")
}else{
    print("Não existe números armazenados")
}
//fazendo a mesma coisa acima um poco diferente
//se não ouver número armazenado, vai receber 999
let ages3 = ages.last ?? 999

//outra forma é usar o guard que funciona apenas dentro de uma função
func getOldestAge(){
    guard let oldestage = ages.last else {
        return
    }
    print("O último é \(oldestage)")
}

getOldestAge()

let forced = ages.last! //colocando exclamação no final, obriga fazer mesmo se não tiver nada dentro, cuidado ao usar, pode causar erro de execução
